   ...
   $data2=mysqli_fetch_assoc($run2);
    ...
    if(mysqli_num_rows($run)>0)
    {
      $row=mysqli_fetch_assoc($run);
        $html="<table style='border:1px solid border'>";
        ...
        $html.="<table>
              <tr>
                  ...
                  <th><span>$all=$data2[maths]+$data2[dd]+$data2[os]+$data2[dbms]+$data2[lab]</span></th>    
              </tr>    
             
          </table>";
        
       $html.="<h3>You Are <span> if($data2[maths]<30 OR $data2[dd]<30 OR $data2[os]<30 OR $data2[dbms]<30 OR $data2[lab]<30) 
        {
            echo 'Fail;
        }
       else
